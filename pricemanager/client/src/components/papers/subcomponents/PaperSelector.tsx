...
export default function PaperSelector({ papers, onPaperSelect = (f: string) => f }: { papers: PaperType[], onPaperSelect: any }) {
    return (
        <div className="flex flex-col border-gray-500 border-2 items-center p-5 bg-blue-100 rounded-lg">
            ...
            <select defaultValue="" className="cursor-pointer block border rounded-lg border-black w-full text-lg p-3 text-center" onChange={(e: ChangeEvent<HTMLSelectElement>) => onPaperSelect(papers.filter((paper: PaperType) => paper.name === e.target.value))}>
